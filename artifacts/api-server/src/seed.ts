...
import { tagProductBusinessTypes } from "./lib/business-packs.js";
import { eq, isNull, and, sql } from "drizzle-orm";

// mysql2 `db.execute` returns [rows, fields]; normalize to a plain rows array.
function rowsOf(r: unknown): any[] {
  if (Array.isArray(r) && Array.isArray((r as any)[0])) return (r as any)[0];
  if (Array.isArray(r)) return r as any[];
  return ((r as any)?.rows ?? []) as any[];
}

// MySQL has no portable `ADD COLUMN IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS`
// (only MariaDB does), so we probe information_schema to stay idempotent on both.
async function columnExists(table: string, column: string): Promise<boolean> {
  const r = await db.execute(sql`
    SELECT COUNT(*) AS c FROM information_schema.columns
    WHERE table_schema = DATABASE() AND table_name = ${table} AND column_name = ${column}
  `);
  return Number(rowsOf(r)[0]?.c ?? 0) > 0;
}
...
// Idempotent, additive schema migrations applied on every boot. Lets a deployed
// database (e.g. Hostinger, which does not run `drizzle-kit push`) pick up new
// columns automatically without a manual SQL step.
async function ensureSchema() {
  try {
    if (!(await columnExists("sales", "original_sale_id"))) await db.execute(sql`ALTER TABLE sales ADD COLUMN original_sale_id int`);
    if (!(await columnExists("sales", "created_by_id"))) await db.execute(sql`ALTER TABLE sales ADD COLUMN created_by_id int`);
    if (!(await columnExists("purchases", "created_by_id"))) await db.execute(sql`ALTER TABLE purchases ADD COLUMN created_by_id int`);
    // Tags each product with the business type that owns it, so the store and
    // customer app can show only the active business's catalog. Must be varchar
    // (not text): it matches the Drizzle schema and MySQL cannot index a TEXT
    // column without a prefix length, which the composite index below needs.
    if (!(await columnExists("products", "business_type"))) await db.execute(sql`ALTER TABLE products ADD COLUMN business_type varchar(50)`);
    // Customer account: available advance credit (store holds) + cumulative advance
    // applied/refunded (audit). Receivable stays live-derived from sales.
    if (!(await columnExists("customers", "advance_balance"))) await db.execute(sql`ALTER TABLE customers ADD COLUMN advance_balance decimal(12,2) NOT NULL DEFAULT 0`);
    if (!(await columnExists("customers", "advance_paid_balance"))) await db.execute(sql`ALTER TABLE customers ADD COLUMN advance_paid_balance decimal(12,2) NOT NULL DEFAULT 0`);
    // Supplier account: advance we have paid the supplier + cumulative advance
    // refunded/applied (audit). Payable stays live-derived from purchases/payments.
    if (!(await columnExists("suppliers", "advance_balance"))) await db.execute(sql`ALTER TABLE suppliers ADD COLUMN advance_balance decimal(12,2) NOT NULL DEFAULT 0`);
    if (!(await columnExists("suppliers", "advance_paid_balance"))) await db.execute(sql`ALTER TABLE suppliers ADD COLUMN advance_paid_balance decimal(12,2) NOT NULL DEFAULT 0`);
    // Customer money movements outside sales/returns (payments, advances, adjustments).
    await db.execute(sql`CREATE TABLE IF NOT EXISTS customer_transactions (
    ...
    try {
      if (!(await indexExists("customer_transactions", "customer_txn_customer_idx"))) {
        await db.execute(sql`CREATE INDEX customer_txn_customer_idx ON customer_transactions (customer_id)`);
      }
      if (!(await indexExists("customer_transactions", "customer_txn_date_idx"))) {
        await db.execute(sql`CREATE INDEX customer_txn_date_idx ON customer_transactions (txn_date)`);
      }
    } catch (err) {
      logger.error({ err }, "ensureSchema: failed to create customer_transactions indexes");
    }
    // Supplier money movements outside purchases/returns (payments, advances, adjustments).
    await db.execute(sql`CREATE TABLE IF NOT EXISTS supplier_transactions (
      id int AUTO_INCREMENT PRIMARY KEY,
    ...
    try {
      if (!(await indexExists("supplier_transactions", "supplier_txn_supplier_idx"))) {
        await db.execute(sql`CREATE INDEX supplier_txn_supplier_idx ON supplier_transactions (supplier_id)`);
      }
      if (!(await indexExists("supplier_transactions", "supplier_txn_date_idx"))) {
        await db.execute(sql`CREATE INDEX supplier_txn_date_idx ON supplier_transactions (txn_date)`);
      }
    } catch (err) {
      logger.error({ err }, "ensureSchema: failed to create supplier_transactions indexes");
    }
    // WhatsApp message log (append-only audit of every send attempt).
    await db.execute(sql`CREATE TABLE IF NOT EXISTS whatsapp_logs (
      id int AUTO_INCREMENT PRIMARY KEY,
    ...
    try {
      if (!(await indexExists("whatsapp_logs", "whatsapp_log_created_idx"))) {
        await db.execute(sql`CREATE INDEX whatsapp_log_created_idx ON whatsapp_logs (created_at)`);
      }
      if (!(await indexExists("whatsapp_logs", "whatsapp_log_entity_idx"))) {
        await db.execute(sql`CREATE INDEX whatsapp_log_entity_idx ON whatsapp_logs (entity_type, entity_id)`);
      }
    } catch (err) {
      logger.error({ err }, "ensureSchema: failed to create whatsapp_logs indexes");
    }
    // Bilingual + business scoping for templates; preferred language on the gateway.
    if (!(await columnExists("whatsapp_templates", "language"))) await db.execute(sql`ALTER TABLE whatsapp_templates ADD COLUMN language varchar(5) NOT NULL DEFAULT 'en'`);
    if (!(await columnExists("whatsapp_templates", "business_type"))) await db.execute(sql`ALTER TABLE whatsapp_templates ADD COLUMN business_type varchar(50)`);
    // Isolate index creation: a failure here must not abort the singleton-table
    // repairs that follow (one broad try/catch would otherwise swallow them).
    try {
      if (!(await indexExists("products", "products_status_business_type_idx"))) {
        await db.execute(sql`CREATE INDEX products_status_business_type_idx ON products (status, business_type)`);
      }
    } catch (err) {
      logger.error({ err }, "ensureSchema: failed to create products_status_business_type_idx");
    }
    // Provider-independent WhatsApp gateway config (single row).
    await db.execute(sql`CREATE TABLE IF NOT EXISTS whatsapp_config (
      id int AUTO_INCREMENT PRIMARY KEY,
    ...
    // Existing installs predate default_language; add it after the table is guaranteed.
    if (!(await columnExists("whatsapp_config", "default_language"))) await db.execute(sql`ALTER TABLE whatsapp_config ADD COLUMN default_language varchar(5) NOT NULL DEFAULT 'en'`);
    // Collapse any accidental duplicate rows into the canonical singleton (id=1)
    // and guarantee that row always exists. All reads/writes target id=1, so no
    // runtime inserts ever happen and duplicates cannot accumulate.
    await db.execute(sql`DELETE w FROM whatsapp_config w JOIN (SELECT MIN(id) AS keep FROM whatsapp_config) m ON w.id <> m.keep`);
    await db.execute(sql`UPDATE whatsapp_config SET id = 1 WHERE id <> 1`);
    await db.execute(sql`INSERT IGNORE INTO whatsapp_config (id, custom_body_template) VALUES (1, '{"to":"{{to}}","message":"{{message}}"}')`);
    // Multi-business platform config (single row, id=1). Missing module keys
    // default to enabled at runtime so existing installs keep every feature.
    await db.execute(sql`CREATE TABLE IF NOT EXISTS business_config (
      id int AUTO_INCREMENT PRIMARY KEY,
    ...
    await db.execute(sql`DELETE b FROM business_config b JOIN (SELECT MIN(id) AS keep FROM business_config) m ON b.id <> m.keep`);
    await db.execute(sql`UPDATE business_config SET id = 1 WHERE id <> 1`);
    await db.execute(sql`INSERT IGNORE INTO business_config (id, active_business_types, enabled_modules, applied_packs) VALUES (1, '[]', '{}', '[]')`);
    ...
    // Backfill business_type on any product still missing it (existing installs).
...
export async function seed() {
  ...
  // Main branch
  const [{ id: mainBranchId }] = await db.insert(branchesTable).values({
    name: "Main Branch",
...
async function seedBookCatalog() {
  ...
  await db.insert(bookClassesTable).values(classData);
  ...
  await db.insert(bookSubjectsTable).values(subjects.map(name => ({ name })));
  ...
  for (const pName of publisherNames) {
    ...
    if (!existing) {
      await db.insert(brandsTable).values({ name: pName, description: "Book publisher" });
    }
  }
  ...
  if (seriesData.length > 0) {
    await db.insert(publisherSeriesTable).values(seriesData);
  }
...
async function seedStationeryCatalog() {
  ...
  if (!stationeryCat) {
    [stationeryCat] = await db.insert(categoriesTable).values({ name: "Stationery", description: "School & office stationery items" }).$returningId();
  }
  ...
  const insertedSubIds = await db.insert(subcategoriesTable)
    .values(subcatNames.map(name => ({ name, categoryId: stationeryId })))
    .$returningId();
  const subId = new Map(subcatNames.map((name, i) => [name, insertedSubIds[i].id]));
