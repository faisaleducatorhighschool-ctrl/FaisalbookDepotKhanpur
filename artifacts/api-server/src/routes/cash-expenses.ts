...
router.post("/cash-collections", requireAuth, async (req, res): Promise<void> => {
  ...
  const [row] = await db.insert(cashCollectionsTable).values({
...
router.patch("/cash-collections/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [row] = await db.update(cashCollectionsTable).set(parsed.data).where(eq(cashCollectionsTable.id, params.data.id)).returning();
...
router.patch("/expenses/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  const [row] = await db.update(expensesTable).set(upd).where(eq(expensesTable.id, params.data.id)).returning();
