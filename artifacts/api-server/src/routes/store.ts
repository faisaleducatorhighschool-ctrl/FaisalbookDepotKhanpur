...
import { eq, and, ilike, or, sql, gte, lte, desc, inArray } from "drizzle-orm";
import {
  db,
  productsTable,
  categoriesTable,
...
router.get("/store/categories", async (_req, res): Promise<void> => {
  const categories = await db
    .select({
      ...
      productCount: sql<number>`(select count(*)::int from products where category_id = categories.id and status = 'active')`,
...
router.get("/store/brands", async (_req, res): Promise<void> => {
  const brands = await db
    .select({
      ...
      productCount: sql<number>`(select count(*)::int from products where brand_id = brands.id and status = 'active')`,
...
router.get("/store/featured", async (_req, res): Promise<void> => {
  ...
  const activeCondition = eq(productsTable.status, "active");
  ...
  const bestSelling = await db
    .select({
      ...productFields,
      totalSold: sql<number>`coalesce((select sum(quantity) from order_items where product_id = products.id), 0)::int`,
    })
    .from(productsTable)
    .where(activeCondition)
...
router.post("/store/orders", optionalCustomerAuth, async (req, res): Promise<void> => {
  ...
  const [{ maxNum }] = await db
    .select({ maxNum: sql<number>`coalesce(max(cast(substring(order_number from 5) as integer)), 0)` })
    .from(ordersTable)
