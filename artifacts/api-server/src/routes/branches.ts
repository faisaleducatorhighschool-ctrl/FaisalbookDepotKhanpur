...
router.post("/branches", requireAuth, async (req, res): Promise<void> => {
  ...
  const [b] = await db.insert(branchesTable).values({ name, address, phone: phone ?? null, email: email ?? null, isMain: isMain ?? false }).returning();
...
router.patch("/branches/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [b] = await db.update(branchesTable).set(parsed.data).where(eq(branchesTable.id, params.data.id)).returning();
