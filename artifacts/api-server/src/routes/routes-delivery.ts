...
router.patch("/routes/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [route] = await db.update(deliveryRoutesTable).set(parsed.data).where(eq(deliveryRoutesTable.id, params.data.id)).returning();
...
router.post("/deliveries", requireAuth, async (req, res): Promise<void> => {
  ...
  const [d] = await db.insert(deliveryAssignmentsTable).values({
...
router.patch("/deliveries/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  const [d] = await db.update(deliveryAssignmentsTable).set(upd).where(eq(deliveryAssignmentsTable.id, params.data.id)).returning();
