...
router.post("/publisher-series", requireAuth, async (req, res): Promise<void> => {
  ...
  const [row] = await db.insert(publisherSeriesTable).values({
...
router.patch("/publisher-series/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  const [row] = await db.update(publisherSeriesTable)
...
router.post("/book-classes", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [row] = await db.insert(bookClassesTable).values({
    name: parsed.data.name,
    sortOrder: parsed.data.sortOrder ?? 0,
  }).onConflictDoNothing().returning();
  if (!row) { res.status(409).json({ error: "Class already exists" }); return; }
...
router.post("/book-subjects", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [row] = await db.insert(bookSubjectsTable).values({ name: parsed.data.name })
    .onConflictDoNothing().returning();
  if (!row) { res.status(409).json({ error: "Subject already exists" }); return; }
  res.status(201).json({ ...row, createdAt: row.createdAt.toISOString() });
});
