import { useGetDashboardStats, useGetRecentOrders, useGetSalesChart, useGetBusinessModules, getGetBusinessModulesQueryKey } from "@workspace/api-client-react";
...
export default function Dashboard() {
  ...
  const primaryLabel = (businessModules as any)?.primaryBusinessLabel as string | null | undefined;
  const primaryType = (businessModules as any)?.primaryBusinessType as string | null | undefined;
  const primaryWidgets = ((businessModules as any)?.primaryWidgets ?? []) as string[];
  ...
  // Show the perishables/expiry section only for businesses that track it (e.g.
  // grocery, pharmacy). When no primary business is chosen, keep showing it.
  const showExpiry = !primaryType || primaryWidgets.includes("expiry");
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {primaryLabel && (
          <Badge variant="outline" className="text-sm py-1 px-3">{primaryLabel}</Badge>
        )}
      </div>
