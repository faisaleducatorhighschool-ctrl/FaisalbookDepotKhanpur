...
export default function Login() {
  ...
  const [branding, setBranding] = useState<{ storeName?: string; logoUrl?: string }>({});

  useEffect(() => {
    fetch(`${BASE}/api/store/settings`)
      .then(r => r.ok ? r.json() : null)
      .then((data) => {
        if (data) setBranding({ storeName: data.storeName, logoUrl: data.logoUrl });
      })
      .catch(() => {});
  }, []);
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-8">
        <div className="flex flex-col items-center justify-center space-y-3">
          ...
          <h1 className="text-3xl font-bold tracking-tight">{branding.storeName || "Tech Mentor ERP & POS"}</h1>
