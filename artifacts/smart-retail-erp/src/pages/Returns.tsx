...
function SaleReturnByInvoice() {
  ...
  const searchInvoice = async () => {
    ...
    try {
      ...
      const remainingTotal = detailed.items.reduce(
        (s: number, it: any) => s + Math.max(0, it.quantity - (it.returnedQuantity ?? 0)), 0);
      if (remainingTotal <= 0) {
        setSearchError("This invoice has already been fully returned.");
        setSearching(false);
        return;
      }
      ...
      setReturnItems(detailed.items.map((it: any) => {
        ...
        const remaining = Math.max(0, it.quantity - returnedQty);
        return {
          productId: it.productId, productName: it.productName,
          quantity: remaining, price: Number(it.price),
          maxQty: remaining, soldQty: it.quantity, returnedQty,
        };
      }));
                  ...
                  <TableBody>
                    {returnItems.map((it, idx) => (
                      <TableRow key={idx} className={it.quantity === 0 ? "opacity-40" : ""}>
                        ...
                        <TableCell className="text-right">{it.soldQty}</TableCell>
                        <TableCell className="text-right text-muted-foreground">{it.returnedQty}</TableCell>
                        <TableCell className="text-right font-medium">{it.maxQty}</TableCell>
                        <TableCell className="text-right">
                          <Input type="number" min={0} max={it.maxQty} value={it.quantity}
                            disabled={it.maxQty === 0}
...
function SaleReturnManual() {
  ...
  const addProduct = (p: any) => {
    ...
    setItems(prev => [...prev, { productId: p.id, productName: p.name, quantity: 1, price: Number(p.salePrice), maxQty: 999, soldQty: 0, returnedQty: 0 }]);
    setSearch("");
  };
...
function PurchaseReturnByPO() {
  ...
  const searchPO = async () => {
    ...
    try {
      ...
      const products: any[] = await apiFetch("/api/products");
      const stockById = new Map<number, number>(products.map(p => [p.id, Number(p.stock ?? 0)]));
      setFoundPO(detailed);
      setReturnItems(detailed.items.map((it: any) => {
        const inStock = stockById.get(it.productId) ?? 0;
        const alreadyReturned = Number(it.returnedQuantity ?? 0);
        const poRemaining = Math.max(0, it.quantity - alreadyReturned);
        // Can't return more than is left on this PO, nor more than is still in
        // stock — sold (or already-returned) units are gone.
        const maxQty = Math.min(poRemaining, inStock);
        return {
          productId: it.productId, productName: it.productName ?? "Unknown",
          quantity: maxQty, costPrice: Number(it.costPrice), maxQty,
          purchasedQty: it.quantity, inStock,
        };
      }));
    } catch (e: any) { setSearchError(e.message); }
    finally { setSearching(false); }
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="md:col-span-2 space-y-4">
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground mb-3">You can only return units still in stock — items already sold can't be returned to the supplier.</p>
                <Table>
                  <TableHeader><TableRow>
                    <TableHead>Product</TableHead>
                  ...
                  <TableBody>
                    {returnItems.map((it, idx) => (
                      <TableRow key={idx} className={it.quantity === 0 ? "opacity-40" : ""}>
                        ...
                        <TableCell className="text-right text-muted-foreground">{it.purchasedQty}</TableCell>
                        <TableCell className="text-right text-muted-foreground">{it.inStock}</TableCell>
                        <TableCell className="text-right font-medium">{it.maxQty}</TableCell>
                        <TableCell className="text-right">
                          <Input type="number" min={0} max={it.maxQty} value={it.quantity}
                            disabled={it.maxQty === 0}
...
function PurchaseReturnManual() {
  ...
  const addProduct = (p: any) => {
    ...
    if (inStock <= 0) {
      toast({ title: "Out of stock", description: `${p.name} has no stock available to return.`, variant: "destructive" });
      return;
    }
    setItems(prev => [...prev, { productId: p.id, productName: p.name, quantity: 1, costPrice: Number(p.costPrice), maxQty: inStock, purchasedQty: 0, inStock }]);
    setSearch("");
  };

  const updateQty = (idx: number, qty: number) => setItems(prev => prev.map((it, i) => i === idx ? { ...it, quantity: Math.min(Math.max(1, qty), it.maxQty) } : it));
